...
# ctk.set_default_color_theme("blue")  # Can change theme
...
#         # Download type selection (more spacing, bigger font)
...
#         self.playlist_radio = ctk.CTkRadioButton(radio_frame, text="Audio Playlist", variable=self.dl_type, value="playlist", font=ctk.CTkFont(size=14))
#         self.video_radio.pack(side="left", padx=25, pady=10)
#         self.audio_radio.pack(side="left", padx=25, pady=10)
#         self.playlist_radio.pack(side="left", padx=25, pady=10)
...
#         self.loading_bar.pack_forget()  # Hide at startup

#         # Folder selection in a row
...
#                     formats = [f for f in first_video['formats'] if f.get('acodec') != 'none' and f.get('vcodec') == 'none' and f.get('abr')]
#                     display_list = [f"{f.get('abr')} kbps {f['ext']}" for f in formats]
#                     format_map = {f"{f.get('abr')} kbps {f['ext']}": f['format_id'] for f in formats}
#                 elif dltype == "audio":
#                     info = ydl.extract_info(url, download=False)
#                     formats = [f for f in info['formats'] if f.get('acodec') != 'none' and f.get('vcodec') == 'none' and f.get('abr')]
#                     display_list = [f"{f.get('abr')} kbps {f['ext']}" for f in formats]
#                     format_map = {f"{f.get('abr')} kbps {f['ext']}": f['format_id'] for f in formats}
#                 else:  # video
#                     info = ydl.extract_info(url, download=False)
...
#                     format_map = {}
#                     for f in formats:
#                         kind = "video+audio" if f.get('acodec') != 'none' else "video only"
#                         quality = f"{f.get('height')}p {kind} {f['ext']}"
#                         display_list.append(quality)
#                         format_map[quality] = f['format_id']
#                 if not display_list:
...
#                 self.status_label.configure(text=f"Downloading... {d['_percent_str']} {d['_speed_str']}", text_color="blue")
...
#                     display_list = [f"{f.get('abr')} kbps {f['ext']}" for f in formats]
#                     format_map = {f"{f.get('abr')} kbps {f['ext']}": f['format_id'] for f in formats}
#                 else:  # video: always provide all heights, always merge with audio
#                     info = ydl.extract_info(url, download=False)
#                     formats = [
#                         f for f in info['formats']
#                         if f.get('height') and f.get('vcodec') != 'none'
...
#                             # Use yt-dlp's bestvideo+bestaudio merging for that height
...
#                 # Only set to "Processing..." here, not open file!
#                 self.after(0, lambda: self.status_label.configure(text="Processing...", text_color="orange"))

#         def post_hook(d):
#             # This really fires after post-processing (e.g., merging or conversion)
#             if d['status'] == 'finished':
#                 last_filename[0] = d.get('filename')
#                 if not opened[0]:
#                     opened[0] = True
#                     self.after(0, lambda: self.status_label.configure(
#                         text="✅ Download complete! Playing...", text_color="green"))
#                     # Wait a tiny bit to ensure file is ready, then open
...
#     # Path to ffmpeg.exe in same dir as app
...
#         def post_hook(d):
#             if d['status'] == 'finished':
#                 # 1. Try to get the actual written file
#                 actual_file = d.get('filepath') or d.get('filename')
#                 # 2. As a fallback, scan folder for the newest audio or video file
